...
import { SongRequest } from '@/app/components/SongRequest';
import { CertificateGallery } from '@/app/components/Certificate';
...
const VALID_VIEWS = new Set(['home','courses','instructors','community','profile','schedule','settings','help','achievements','scanner','stage','admin-dash','instructor-dash','feedback-review','instructor-lessons','teacher-reviews','users-manage','messages','ensemble','content-approve','upload-lesson','course-detail','rubrics','upload-guide','royalties','hall-of-fame','song-requests','certificates','parent-dash']);
...
const App = () => {
  ...
  const handleLogin = (selectedRole: UserRole) => {
    ...
    else if (selectedRole === 'parent') setCurrentView('parent-dash');
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case 'song-requests':
        return role !== 'guest' ? <SongRequest /> : <AccessDenied requiredRole="student or instructor" />;
      case 'certificates':
        return role !== 'guest' ? <CertificateGallery /> : <AccessDenied requiredRole="student" />;
      case 'parent-dash':
        return role === 'parent' ? <ParentDashboard /> : <AccessDenied requiredRole="parent" />;
      case 'home':
      default:
        if (role === 'guest') {
          return (
